...
import { Search, Filter, School, ArrowUpDown } from "lucide-react"
...
export default function AdmissionScorePage({ admissionScore }: AdmissionScorePageProps) {
  ...
  const { parsedData, columns, filterableColumns } = useMemo(() => {
    if (!admissionScore.tableData) return { parsedData: [], columns: [], filterableColumns: [] }
    
    try {
      const lines = admissionScore.tableData.trim().split('\n')
      if (lines.length < 2) return { parsedData: [], columns: [], filterableColumns: [] }
      
      // Parse header line to get column names
      const headerLine = lines[0]
      const headerMatches = headerLine.match(/(".*?"|[^,]+)(?=\s*,|\s*$)/g)
      if (!headerMatches) return { parsedData: [], columns: [], filterableColumns: [] }
      ...
      const columns: ColumnInfo[] = headers.map((header, index) => {
        ...
        if (lowerHeader.includes('puan') || lowerHeader.includes('sıra') || 
            lowerHeader.includes('kontenjan') || lowerHeader.includes('sayı')) {
          type = 'number'
        } else if (lowerHeader.includes('tür') || lowerHeader.includes('type') ||
      ...
      const parsedData: DynamicTableData[] = dataLines.map((line, index) => {
  ...
  const filterOptions = useMemo(() => {
    ...
    filterableColumns.forEach(col => {
      const uniqueValues = [...new Set(parsedData.map(row => row[col.key]))]
      options[col.key] = uniqueValues.filter((value): value is string => 
  ...
  const filteredData = useMemo(() => {
    ...
    return filtered
  }, [parsedData, searchTerm, selectedFilters, sortField, sortDirection, columns, filterableColumns])
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      {parsedData.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">Bu sayfa için henüz tablo verisi mevcut değil.</p>
        </div>
      )}
        ...
        <>
          ...
          <section className="mb-8">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              <h2 className="text-2xl font-semibold text-gray-900">{admissionScore.title}</h2>
          ...
          <div className="mb-4">
            <p className="text-gray-600">
              <span className="font-semibold">{filteredData.length}</span> sonuç bulundu
                    ...
                    <TableHeader>
                      <TableRow>
                        {columns.map((column, index) => (
                          <TableHead 
                            key={column.key}
                            className={`cursor-pointer hover:bg-gray-50 ${index === 0 ? 'text-left pl-4' : 'text-center'}`}
                            onClick={() => handleSort(column.key)}
                          >
                            <div className={`flex items-center space-x-2 ${index === 0 ? 'justify-start' : 'justify-center'}`}>
                    ...
                    <TableBody>
                      {filteredData.map((row) => (
                        <TableRow key={row.id} className="hover:bg-gray-50">
                          {columns.map((column, index) => (
                            <TableCell key={column.key} className={index === 0 ? 'text-left pl-4' : 'text-center'}>
